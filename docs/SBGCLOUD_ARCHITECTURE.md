...
|---|---|---|---|---|---|
| `PosDevice` | Identity of a physical till/tablet: which tenant/store it belongs to, its device type, current app version, and activation state. | `id`, `tenantId`, `storeId`, `name`, `deviceType`, `status` (PENDING/ACTIVE/SUSPENDED/REVOKED), `apiKeyHash`/`apiKeyPrefix`, `deviceCode` (public, non-secret, unique per tenant — checked as a pair with `X-Device-Code` against the token's device, docs/POS_SYNC_API.md §4), `lastSeenAt` | store-admin `POST /pos-devices` (create); POS Sync `/activate` (confirms, sets credential + required `deviceCode`, rejects a collision with another active device in the tenant as `409`); `/heartbeat` (`lastSeenAt`) | POS Sync device auth (`resolveDevice` for the credential, `resolveAuthenticatedDevice` for the `deviceCode` pairing check on every other endpoint) | Leaked `apiKeyHash` input (raw key) = device impersonation; a caller must check `status === ACTIVE`, not just "hash matches". `deviceCode`'s origin (device-generated, confirmed with the Android team) and the dual-header requirement are both settled — see POS_SYNC_API.md §4/§22 |
