...
## 14. Staff / operators (implemented, additive extension)

Adds an optional `staff` block to `GET /pos/v1/settings`, agreed with the
Android team as a fourth independently-versioned block alongside
`settings`/`policies`/`printTemplates`. Implemented in the same
`pos-sync` module — `packages/prisma/schema.prisma` (`PosOperator` model),
`apps/api/src/modules/pos-sync/routes.ts` (GET /settings), and
`apps/api/src/modules/pos-sync/admin-routes.ts` (store-admin CRUD).

### 14.1 `PosOperator` — data model

Deliberately a **separate entity, not a role/permission variant of
`User`/`Team`** — a cashier working a till has no login to the
tenant-admin panel and no `User` row at all. Store-scoped like
`PosDevice`/`PosSettings`:
...
`id` is always a real CUID generated by this table — never an arbitrary
string a client (till or store-admin) supplies. **PIN/password auth is
explicitly out of scope for this iteration** — this is a roster +
permissions list only; a till authenticating a specific cashier by
PIN/password against this list is future work.

### 14.2 Versioning

A fourth independent counter, same pattern as §5: `staffVersion` —
`PosSettings.staffVersion`, a plain `Int @default(1)` column, bumped by
`{ increment: 1 }` on every `PosOperator` create/update/delete for that
store (mirrors `settingsVersion`'s bump-on-write mechanism, not a
computed sum like `policiesVersion`). If a store has no `PosSettings` row
yet, the first staff write creates one with the same eight-key-default
`payload` `GET /settings` already treats as "unconfigured" (§6) — staff
management does not depend on settings having been configured first.

### 14.3 `GET /pos/v1/settings` — `staff` block
      ...
      "operators": [
        {
          ...
          "name": "Alice",
          "role": "senior_cashier",
          "permissions": ["void_sale", "apply_discount"],
          "active": true
        }
      ]
    }
...
**`staff` is optional and nullable, by design:**

- A store with **no** `PosOperator` rows (or none `active: true`) gets
  `"staff": null` — never `{ "operators": [] }` — so the till can tell
  "no operators configured, use your local fallback roster" apart from
  "the server sent an explicitly empty roster." Android falls back to a
  device-local operator list in the `null` case.
- Only `active: true` operators are ever included, sorted by `name`
  ascending — same "never send a disabled/inactive row to a till at all"
  convention §7 already uses for `enabled: false` policy rules.
- `role` is stored uppercase (`PosOperatorRole` enum:
  `CASHIER`/`SENIOR_CASHIER`/`ADMIN`) but served **lowercase** on the
  wire (`cashier`/`senior_cashier`/`admin`) — the exact casing the
  Android team's contract expects. This is a response-serialization
  detail only; nothing else in this document's enums (e.g.
  `PolicyScope`, `PolicySeverity`) changes casing convention — that
  choice is specific to this field, agreed with Android separately.

### 14.4 Store-admin CRUD (`/api/store-admin/pos-operators`)

All four endpoints require `permissionGuard('manageSettings')` — managing
till cashier staff is a store-configuration action, not a distinct
permission of its own — plus the existing `planGuard('posEnabled')` every
other endpoint in this admin module uses.

| Method | Path | Body / query | Notes |
|---|---|---|---|
| `GET` | `/pos-operators?storeId=...` | — | Lists **all** operators for the store (active and inactive) — a management view, unlike the till-facing `GET /settings` which filters to `active: true` only. |
| `POST` | `/pos-operators` | `{ storeId, name, role, permissions?, active? }` | `role` uppercase, matching the enum. `permissions` defaults to `[]`, `active` defaults to `true`. |
| `PATCH` | `/pos-operators/:id` | any subset of `{ name, role, permissions, active }` | Partial update. |
| `DELETE` | `/pos-operators/:id` | — | Hard delete. |

Every write (`POST`/`PATCH`/`DELETE`) bumps `PosSettings.staffVersion`
for the operator's store (§14.2) unconditionally on success.

**Tenant isolation**: `PATCH`/`DELETE` look up the operator scoped to
`{ id, tenantId }` first — an operator that exists but belongs to a
different tenant is a `404`, never a `403`, matching the
`GET /pos-devices` / `command ack` precedent elsewhere in this module (an
existing-but-foreign resource must not leak as "found").

### 14.5 Backward compatibility

Purely additive, same guarantee as every other block in this document:

- New `pos_operators` table, new `PosOperatorRole` enum, one new
  `staffVersion Int @default(1)` column on `pos_settings` — no existing
  column altered or dropped, no data backfilled.
- A device/till built against the pre-staff contract simply doesn't read
  the new `staffVersion`/`staff` keys — the rest of the `GET /settings`
  response is byte-for-byte unchanged.
- A store that predates this feature (no `PosOperator` rows) gets
  `staffVersion: 1` and `staff: null`, exactly like an unconfigured
