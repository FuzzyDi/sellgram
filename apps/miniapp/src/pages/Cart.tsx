  ...
  return (
    <div className="anim-fade" style={{ paddingBottom: items.length ? 'calc(var(--nav-h) + 80px)' : 'calc(var(--nav-h) + 12px)' }}>
      <div className="glass sticky-header">
        <h1 className="page-title">{tr('Корзина', 'Savat')}</h1>
      </div>

      {items.length === 0 ? (
        <div className="anim-scale empty-state">
          <div className="empty-icon">🛒</div>
          <p className="empty-title">{tr('Корзина пуста', "Savat bo'sh")}</p>
          <p className="empty-sub">{tr('Добавьте товары из каталога', "Katalogdan mahsulot qo'shing")}</p>
        ...
        <div className="list-pad">
          {items.map((item: any, i: number) => (
            <div key={item.id} className={`row-card anim-fade anim-d${Math.min(i, 5)}`}>
              <div className="thumb">
                {item.image ? <img src={item.image} /> : <div className="thumb-fallback"><span>📦</span></div>}
              </div>
              <div className="flex-fill">
                <p className="row-title">{item.name}</p>
                <p className="row-price">{item.price.toLocaleString()} <span className="muted-sm">{tr('сум', "so'm")}</span></p>
              </div>
      ...
      {items.length > 0 && (
        <div className="glass" style={{ position: 'fixed', bottom: 'var(--nav-h)', left: 0, right: 0, zIndex: 30, padding: '10px 16px 10px', borderTop: '0.5px solid var(--divider)' }}>
          <div className="row-between" style={{ marginBottom: 8 }}>
            <span style={{ fontSize: 15 }} className="muted">{items.length} {tr('товар(ов)', 'ta mahsulot')}</span>
