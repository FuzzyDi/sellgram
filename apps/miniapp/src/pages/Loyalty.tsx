  ...
  return (
    <div className="anim-fade pb-nav">
      <div className="glass sticky-header">
        <h1 className="page-title">{tr('Баллы', 'Ballar')}</h1>
      </div>
      ...
      {txns.length > 0 && (
        <div style={{ padding: '0 12px' }}>
          ...
          {txns.map((t: any, i: number) => (
            <div key={i} className={`row-between anim-fade anim-d${Math.min(i, 5)}`} style={{ padding: '12px 14px', background: 'var(--sec)', borderRadius: 'var(--radius)', marginBottom: 6 }}>
              <div>
                <p style={{ fontSize: 14, fontWeight: 500 }}>{t.description}</p>
                <p className="muted" style={{ fontSize: 12, marginTop: 2 }}>{new Date(t.createdAt).toLocaleDateString(locale)}</p>
              </div>
